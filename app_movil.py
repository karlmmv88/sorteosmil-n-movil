...
def main():
                            ...
                            with st.form("venta_single"):
                                ...
                                if st.form_submit_button("💾 ASIGNAR", use_container_width=True):
                                    if nom_sel:
                                        ...
                                        log_movimiento(id_sorteo, est, f"{str_num}||{nom_sel}", abono)
    ...
    with tab_cobranza:
        ...
        # 1. ESTADO ACTUAL (Tus 90 boletos)
        sql_estado = """
            SELECT b.numero, c.nombre_completo, c.telefono, c.cedula, UPPER(b.estado), 
                   b.precio, b.total_abonado, (b.precio - b.total_abonado), b.fecha_asignacion
            FROM boletos b
        ...
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            ...
            if rows_estado:
                df_estado = pd.DataFrame(rows_estado, columns=["Número", "Cliente", "Teléfono", "Cédula", "Estado", "Precio", "Abonado", "Saldo", "Fecha Asignación"])
                ...
                pd.DataFrame(columns=["Sin Datos"]).to_excel(writer, sheet_name='Estado General', index=False)

            # --- HOJA 2: HISTORIAL (Formateado como pediste) ---
            if rows_hist:
                # Procesamos los datos fila por fila para separar columnas
                data_procesada = []
                for row in rows_hist:
                    f_raw, usuario, detalle_raw, accion_raw, monto = row
                    ...
                    try:
                        ...
                        # Ajuste horario manual si es necesario (ej: -4h)
                    ...
                    # 2. Separar Boletos y Cliente (Usando el separador '||')
                    # Formato esperado en detalle: "10, 20, 30||Juan Perez"
                    boletos_str = detalle_raw
                    cliente_str = ""
                    
                    if "||" in str(detalle_raw):
                        partes = str(detalle_raw).split("||")
                        boletos_str = partes[0].strip()
                        if len(partes) > 1: cliente_str = partes[1].strip()
                    else:
                        # Soporte para registros viejos que no tenían separador
                        cliente_str = "-"
                    ...
                    data_procesada.append([
                        ...
                        boletos_str, 
                        cliente_str, 
                        accion_raw.lower(), # Acción en minúscula
                        monto
                    ])

                cols_finales = ["Fecha", "Hora", "Usuario", "N° boleto", "Nombre cliente", "Acción", "Monto"]
                df_hist = pd.DataFrame(data_procesada, columns=cols_finales)
        ...
        if hay_datos:
            st.download_button(
                ...
                data=buffer,
                file_name=f"Reporte_{nombre_s}.xlsx",
        ...
        st.divider()
        # (El resto del código de Cobranza en Pantalla sigue igual...)
        ...
        if raw_deudores:
            # Lógica de agrupación y visualización (sin cambios)
            grupos = {}
            ...
            for clave, d in grupos.items():
                ...
                with st.container(border=True):
                    ...
                    with c_info:
                        st.markdown(f"👤 **{nom}**")
                        st.caption(f"🎟️: {str_numeros}")
                        st.write(f"🔴: :red[**${d['t_deuda']:,.2f}**]")
                    with c_btn:
                        if tel and len(str(tel)) > 5:
                            ...
                            link = f"https://api.whatsapp.com/send?phone={tel_clean}&text={urllib.parse.quote(f'Hola {nom}, saldo pendiente ${d['t_deuda']:.2f} boletos {str_numeros}')}"
